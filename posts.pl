...
sub ReadPosts {
	...
	$sql = qq{SELECT user_login, UNIX_TIMESTAMP(post_date), post_type, post_content, post_src, post_id from users, posts WHERE post_date > 
FROM_UNIXTIME($timestamp) and user_id = post_user_id ORDER BY post_date asc limit 50};
	$sth = MnmDB::prepare($sql);
	$sth->execute ||  die "Could not execute SQL statement: $sql";
	while (my ($username, $date, $tipo, $content, $src, $postid) = $sth->fetchrow_array) {
		...
		foreach my $u ($jabber->users()) {
			...
			if ($tipo eq  'admin') {
			$jabber->SendMessage($u, "Admin: $content -- Para leerla en la web entra a http://joneame.net/notitas/");
                         }
		# Send the note if the user is the poster is a friend, the same user or has answered him with a @username at the begining
			elsif (($u->friend($poster) > 0 || $u == $poster || $content =~ /(^|\W)\@$username\W/i) && ($tipo ne 'admin')) {
				$jabber->SendMessage($u, "$poster->{user} ($src): $content -- http://joneame.net/notitas/$poster->{user}/$postid ");
			}
		}
	}
}
...
sub InMessage {
	...
	if ($poster->karma < 6.5) {
		$jabber->SendMessage($poster, "no tienes suficiente carisma");
		return;
	}
	chomp($body);
	if ($body =~ /^ *!/) {
	...
	if (length($body) > 350) {
		$jabber->SendMessage($poster, "mensaje demasiado largo (long max: 350)");
		return;
	}
	...
	$sth = MnmDB::prepare(qq{INSERT INTO posts (post_user_id, post_src, post_ip_int, post_randkey, post_content, post_type, post_last_answer) VALUES (?, ?, ?, ?, 
?,?,?) });
	$sth->execute($poster->id, 'im', 0, int(rand(1000000)), $body, 'normal', time);
