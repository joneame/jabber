		...
		if (($link = MnmDB::row_hash($sql))) {
			...
			foreach my $u ($jabber->users()) {
				...
				if ($u->get_pref('jabber-text')) {
					$jabber->SendMessage($u, "$status ($link->{user_login}): $link->{link_title}\n$content http://joneame.net/historia/$link->{link_uri}\n");
				} else {
					$jabber->SendMessage($u, "$status ($link->{user_login}): $link->{link_title}\n http://joneame.net/historia/$link->{link_uri}\n");
				}
			}
		}
	}
	...
	while ($hash = $sth->fetchrow_hashref) {
		...
		foreach my $u ($jabber->users()) {
			...
			if (!$u->get_pref('jabber-off') && $u->get_pref('jabber-chat') && $u != $poster) {
				...
				} elsif ($hash->{chat_room} eq 'devel') {
					if ($u->level eq 'admin' || $u->level eq 'god' || $u->level eq 'devel') {
						$prompt = '%%'.$poster->{user};
						$jabber->SendMessage($u, "$prompt: $content");
					}
...
sub StoreChat {
	...
	#}
	if (length($body) ==  0) {
		$jabber->SendMessage($poster, "no tiene caracteres' :-P");
		return;
	}
	if (length($body) > 250) {
	...
	if ($body =~ /^ *@/) {
		...
		$body =~ s/^ *#//;
		$room = 'admin';
	} elsif ($body =~ /^ *%/ && ($poster->level eq 'god' || $poster->level eq 'admin' || $poster->level eq 'devel')) {
		$body =~ s/^ *%//;
		$room = 'devel';
	}else {
		$room = 'all';
	...
	$sth->execute($now, $poster->id, $room, $poster->user, $body);
        $sth = MnmDB::prepare(qq{insert into chats_logs (chat_time, chat_uid, chat_room, chat_user, chat_text) values (?, ?, ?, ?, ?)});
        $sth->execute($now, $poster->id, $room, $poster->user, $body);
